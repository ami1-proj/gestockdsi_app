...
trait AdresseemailTrait {
     ...
     */
    public function createNew($email, $elem_type, $elem_id) : Adresseemail
    {
        if (empty($email)) {
          return null;
        }
        ...
        $elem = $this->getElem($elem_type, $elem_id);
     ...
     */
    public function moveUp($adresseemail_id, $elem_type, $elem_id) : Adresseemail
    {
        $elem = $this->getElem($elem_type, $elem_id);
        ...
        if ($adresseemail->pivot->rang > 1) {
            ...
            $this->switchRang($elem, ($adresseemail->pivot->rang - 1), $adresseemail->pivot->rang);
   ...
   * Deplace l email d'un element (fournisseur, employe) vers le bas
     ...
     * @return Adresseemail              L email modifie
     */
    public function moveDown($adresseemail_id, $elem_type, $elem_id) : Adresseemail
    {
        $elem = $this->getElem($elem_type, $elem_id);
        $adresseemail = $elem->adresseemails()->where('adresseemail_id', $adresseemail_id)->first();
   ...
   * Supprime l email d un element (fournisseur, employe)
     ...
     */
    public function deleteOne($adresseemail_id, $elem_type, $elem_id) : int
    {
     ...
     */
    private function switchRang($elem, $old_rang, $new_rang) : int
    {
     ...
     */
    private function reorganiseRangs($elem) : int
    {
        $rang = 1;
     ...
     */
    private function getElem($elem_type, $elem_id)
    {
