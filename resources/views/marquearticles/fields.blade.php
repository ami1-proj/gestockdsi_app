<div class="form-group row {{ $errors->has('nom') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
        <input name="nom" type="text" class="form-control" placeholder="Libelle" value="{{ $marquearticle->nom ?? '' }}"/>
    ...
    <div class="col-sm-10">
        <select name="statut_id" class="form-control" id="statut_id" required>
            @foreach($statuts as $id => $display)
                <option value="{{ $id }}" {{ (isset($marquearticles->statut_id) && $id === $marquearticles->statut_id) ? 'selected' : '' }}>{{ $display }}</option>
            @endforeach
        </select>
