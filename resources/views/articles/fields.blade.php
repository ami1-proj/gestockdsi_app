...
<div class="form-group row {{ $errors->has('type_article_id') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
        <select name="type_article_id" class="form-control" id="type_article_id" required>
...
<div class="form-group row {{ $errors->has('fournisseur_id') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
        <select name="fournisseur_id" class="form-control" id="fournisseur_id" required>
            <option selected disabled>Selectionnez un Fournisseur</option>
            @foreach($fournisseurs as $id => $display)
                <option value="{{ $id }}" {{ (isset($article->fournisseur_id) && $id === $article->fournisseur_id) ? 'selected' : '' }}>{{ $display }}</option>
            @endforeach
        </select>
...
<div class="form-group row {{ $errors->has('marque_article_id') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
        <select name="marque_article_id" class="form-control" id="marque_article_id" required>
            <option selected disabled>Selectionnez une Marque</option>
    ...
    <div class="col-sm-10">
        <select name="statut_id" class="form-control" id="statut_id" required>
            @foreach($statuts as $id => $display)
                <option value="{{ $id }}" {{ (isset($article->statut_id) && $id === $article->statut_id) ? 'selected' : '' }}>{{ $display }}</option>
            @endforeach
        </select>
